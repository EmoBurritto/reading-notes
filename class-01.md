...
An HTML element consists of the element with less than sign, element name and greater than sign surrounding it, opening tag. The content or txt of the element. Finally, the closing tag displayed by a less than sign, back slash, name of element and a greater than sign to close the element. 

3. What is the Difference between <article> and <section> element tags?
Articles are used for block of related content that stands on it's own when compared to the rest of the site; like a single blog post. While,  Sections are for grouping multiple related blocks that add function to the whole site; like a mini map. 
...
The type of metadata, like javascript script or CSS links, infulence how the page will look like and act like when accessed by the user. Metadata is the information about the sites initial data on the site's behavior and display. It also helps the engine render the data to the user correctly. 
